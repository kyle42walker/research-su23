namespace Model {
    export class Graph {
        ...
        AddEdge(sourceId: number, targetId: number) {
            ...
            if (!this.isDirected)
                this.edges.push(new Edge(targetId, sourceId))
        }
        ...
        RemoveEdge(sourceId: number, targetId: number) {
            ...
            if (!this.isDirected)
                this.edges = this.edges.filter(e => e.sourceId !== targetId || e.targetId !== sourceId)
        }
        ...
        SetEdgeState(sourceId: number, targetId: number, state: boolean) {
            ...
            if (!this.isDirected)
                this.edges.find(e => e.sourceId === targetId && e.targetId === sourceId)!.isActive = state
        }
